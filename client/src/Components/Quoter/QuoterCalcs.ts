...
const cordCostCalc = (
  ...
  quoterPrices: AxiosResponse<any, any> | undefined
) => {
  ...
  return Math.ceil(
    (bagInfo.bagWidth * 4 + 20) *
      (quoterPrices?.data[0].cordPrice / 100) *
      bagInfo.bagQuantity
  );
};
...
const clothCostCalc = (
  ...
  const bagWxRollW = Math.floor(
    quoterPrices?.data[0].cloth[0].rollWidth / (bagInfo.bagWidth + 2)
  );
  ...
  const bagLxRollW = Math.floor(
    quoterPrices?.data[0].cloth[0].rollWidth / (bagInfo.bagLength * 2 + 5)
  );
  /* Checks if its better to cut bags over the width or the length */
  const clothCentimetersW = Math.ceil(
    ((bagInfo.bagLength * 2 + 5) * bagInfo.bagQuantity) / bagWxRollW
  );
  const clothCentimetersL = Math.ceil(
    ((bagInfo.bagWidth + 2) * bagInfo.bagQuantity) / bagLxRollW
  );
  if (clothCentimetersW < clothCentimetersL) {
    return Math.ceil(
      clothCentimetersW * (quoterPrices?.data[0].cloth[0].price / 100)
    );
  } else {
    return Math.ceil(
      clothCentimetersL * (quoterPrices?.data[0].cloth[0].price / 100)
    );
  }
};
